...
e.prototype.discard = function () {
  if (this._isPickup) {
    ...
    const n = e.node.getPosition().add(cc.v2(0, 120));
    this.node.setPosition(n);
    this.updateAreaKey();
    this.node.active = !0;
    const i = t.level.getRoomById(e.roomId).getGroundY();
    this.updateRoomId(e.roomId);
    const o = $randomUtil.RandomUtil.randomInt(-100, 100);
...
e.prototype.pickup = function (t, e) {
  ...
  if (this._isDroped && !this._isPickup && !this._isRemove) {
    ...
    this._isPickup = !0;
    const o = this.node.convertToWorldSpaceAR(cc.v2(0, 20));
    const r = $battleMgr.default.instance.getCurScene().isPlay;
    switch (this._cfg.type) {
      ...
      default:
        ...
        if (999 == this._cfg.id) {
          $eventManager.EventManager.instance.emit(
            $battleEnum.EBattleEvent.SURVIVOR_UPDATE_INFORM,
            !0,
          );
        }
