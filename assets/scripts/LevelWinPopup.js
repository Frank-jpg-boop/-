...
e.prototype.onEventSwitchBagItemReward = function (t, e, n, i) {
  const o = this;
  const r = this.node.convertToNodeSpaceAR(n);
  const a = function (t, e, n, i, a) {
    const s = [];
    if (n > 40) {
      n = 40;
    }
    for (
      ...
      const d = $nodePoolManager.default.instance.getNode(o.pGameFlyGood);
      d.parent = o.node;
      d.setPosition(r);
  ...
  const s = this._bagCreateRewardMaps[t];
  if (!s || s.size <= 0) {
    if (i) {
      i();
    }
    ...
    s.forEach(function (t, e) {
      const n = 0;
      const r = $cfg.default.instance.dataItem.getById(e);
      if (111 != r.type) {
        if (2 == r.type) {
          n = 2 + r.rare;
        } else {
...
e.prototype.winComplete = function () {
  if (!this._isComplete) {
    this._isComplete = !0;
    const t = $stageDataProxy.stageDataProxy.passStageId;
    ...
    if (t != $stageDataProxy.stageDataProxy.passStageId) {
      ...
      const n = $cfg.default.instance.dataStage.getById(
        $levelBattleData.levelBattleData.cfgStage.id + 1,
      );
      if (n) {
        e =
          '还需救出<color=#EC382E>' +
...
e.prototype.onHide = function () {
  $eventManager.EventManager.instance.off(
    a.SWITCH_BAG_ITEM_REWARD,
    this.onEventSwitchBagItemReward,
    this,
  );
  $eventManager.EventManager.instance.off(
    a.ON_REWARD_SWITCH_COMPLETED,
    this.onEventRewardSwitchComplete,
    this,
  );
};
...
e.prototype.prevReward = function () {
  ...
  if (this._isFinish) {
    $unitMgr.UnitMgr.instance
      .queryUnit($gridAreaDivisionMgr.E_AreaObjectType.SURVIVOR)
      .forEach(function (e) {
        if (e.isRescue) {
          t._rescueCount++;
          const n = $stageDataProxy.stageDataProxy.getStageInfo(
...
e.prototype.init = function (t) {
  ...
  if (t.isFinish) {
    $levelBattleData.levelBattleData.data.exploreValue =
      $levelBattleData.levelBattleData.cfgStage.maxRoom;
  }
  if (
