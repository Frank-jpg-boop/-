...
e.prototype.onBtnUpGerad = function () {
  ...
  let e;
  const n = this;
  const i = $playerDataProxy.playerDataProxy.getArtifactLv(
  ...
  const o = $playerDataProxy.playerDataProxy.getArtifactUpGreadNeedNum(
    this._artifactData.id,
  );
  const r = $playerDataProxy.playerDataProxy.getArtifactUpGreadNeeItemId(
    this._artifactData.id,
  );
  ...
  for (
    const l = this.mSkillItems.children, u = !1, p = null, h = 0;
    h < l.length;
    ++h
  ) {
    const f = l[h];
...
e.prototype.onRemainsItemClick = function (t, e) {
  ...
  const c = $nodeUtil.default.nodeParentChangeLocalPos(
    e.item,
...
e.prototype.initAttributes = function () {
  ...
  const s = $playerDataProxy.playerDataProxy.getArtifactLv(
    this._artifactData.id,
  );
  const c = a[s - 1];
  if (c) {
  ...
  const l = a[s];
  const u = $playerDataProxy.playerDataProxy.getArtifactUpGreadNeedNum(
    this._artifactData.id,
  );
  const p = $playerDataProxy.playerDataProxy.getArtifactUpGreadNeeItemId(
    this._artifactData.id,
  );
...
e.prototype.initMyList = function () {
  for (
    const t = this, e = this._artifactData.chooseLv.split("|"), n = 0;
    n < e.length;
    ++n
  ) {
    const i = e[n];
