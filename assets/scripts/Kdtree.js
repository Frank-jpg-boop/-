...
var c = (function () {
  ...
  t.prototype.buildObstacleTree = function () {
    ...
    for (
      var t = new Array($simulator.Simulator.instance.obstacles.length), e = 0;
      e < t.length;
      e++
    ) {
  ...
  t.prototype.buildAgentTreeRecursive = function (t, e, n) {
    ...
    this.agentTree[n].minX = this.agentTree[n].maxX =
      this.agents[t].position_.x;
    this.agentTree[n].minY = this.agentTree[n].maxY =
      this.agents[t].position_.y;
    for (var i = t + 1; i < e; ++i) {
      this.agentTree[n].maxX = Math.max(
        this.agentTree[n].maxX,
        this.agents[i].position_.x,
      );
      this.agentTree[n].minX = Math.min(
        this.agentTree[n].minX,
        this.agents[i].position_.x,
      );
      this.agentTree[n].maxY = Math.max(
        this.agentTree[n].maxY,
        this.agents[i].position_.y,
      );
      this.agentTree[n].minY = Math.min(
        this.agentTree[n].minY,
        this.agents[i].position_.y,
      );
    }
    if (e - t > this.MAX_LEAF_SIZE) {
      for (
        ...
        for (
          ;
          a < s &&
          (o ? this.agents[a].position_.x : this.agents[a].position_.y) < r;
        ) {
          ++a;
        }
        for (
          ...
          s > a &&
          (o
            ? this.agents[s - 1].position_.x
            : this.agents[s - 1].position_.y) >= r;
        ) {
          --s;
        }
        if (a < s) {
  ...
  t.prototype.buildObstacleTreeRecursive = function (t) {
    ...
    for (
      var e = new s(), n = 0, a = t.length, c = a, l = 0;
      l < t.length;
      ++l
    ) {
      for (var u = 0, p = 0, h = (A = t[l]).next, f = 0; f < t.length; f++) {
        if (l != f) {
          ...
          if (
            m >= -$common.RVOMath.RVO_EPSILON &&
            y >= -$common.RVOMath.RVO_EPSILON
          ) {
            ++u;
          } else {
            (m <= $common.RVOMath.RVO_EPSILON &&
    ...
    for (f = 0; f < t.length; ++f) {
      if (l != f) {
        ...
        if (
          ...
          var I =
            $common.RVOMath.det(
              h.point.minus(A.point),
              M.point.minus(A.point),
            ) /
    ...
    if (this.agentTree[n].end - this.agentTree[n].begin <= this.MAX_LEAF_SIZE) {
      ...
      var r =
        $common.RVOMath.sqr(
          Math.max(
            0,
            this.agentTree[this.agentTree[n].left].minX - t.position_.x,
          ),
        ) +
        $common.RVOMath.sqr(
          Math.max(
            0,
            t.position_.x - this.agentTree[this.agentTree[n].left].maxX,
          ),
        ) +
        $common.RVOMath.sqr(
          Math.max(
            0,
            this.agentTree[this.agentTree[n].left].minY - t.position_.y,
          ),
        ) +
        $common.RVOMath.sqr(
          Math.max(
            0,
            t.position_.y - this.agentTree[this.agentTree[n].left].maxY,
          ),
        );
      var a =
        $common.RVOMath.sqr(
          Math.max(
            0,
            this.agentTree[this.agentTree[n].right].minX - t.position_.x,
          ),
        ) +
        $common.RVOMath.sqr(
          Math.max(
            0,
            t.position_.x - this.agentTree[this.agentTree[n].right].maxX,
          ),
        ) +
        $common.RVOMath.sqr(
          Math.max(
            0,
            this.agentTree[this.agentTree[n].right].minY - t.position_.y,
          ),
        ) +
        $common.RVOMath.sqr(
          Math.max(
            0,
            t.position_.y - this.agentTree[this.agentTree[n].right].maxY,
          ),
        );
      if (r < a) {
        if (
          r < e &&
          a < (e = this.queryAgentTreeRecursive(t, e, this.agentTree[n].left))
        ) {
        ...
        if (
          a < e &&
          r < (e = this.queryAgentTreeRecursive(t, e, this.agentTree[n].right))
        ) {
  ...
  t.prototype.queryObstacleTreeRecursive = function (t, e, n) {
    ...
    if (
      $common.RVOMath.sqr(a) / $common.RVOMath.absSq(r.point.minus(o.point)) <
      e
    ) {
      if (a < 0) {
