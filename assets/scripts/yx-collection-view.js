...
const l = c.ccclass;
const u = c.property;
const p = (c.requireComponent, c.executionOrder);
const h = c.disallowMultiple;
const f = c.help;
const d = cc.Enum;
const m = cc.Node;
const y = cc.Prefab;
const _ = cc.ValueType;
const g = cc.NodePool;
const v = cc.Component;
const b = cc.ScrollView;
const E = (cc.Event.EventMouse, cc.Event.EventTouch, cc.Mask);
const S = cc.Node.EventType;
const P = new cc.Vec3();
const A = new cc.Rect();
const w = new cc.Rect();
(function (t) {
...
e.prototype._yxGetScrollTarget = function (t, n) {
  const i = t.target._yx_scroll_target;
  if (i) {
    return i;
  }
  const o = [t.target];
  ...
  const r = t.touch;
  const a = r.getLocation().subtract(r.getStartLocation());
  const s = Math.abs(a.x);
  const c = Math.abs(a.y);
  if (Math.abs(s - c) < 5) {
    return null;
  }
  for (const l = null, u = 0; u < o.length; u++) {
    const p = (f = o[u]).getComponent(e);
    if (p) {
      const h = f.getComponent(B);
...
e.prototype._onTouchMoved = function (e, n) {
  if (0 != this.node.getComponent(B).scrollEnabled) {
    const i = this._yxGetScrollTarget(e, n);
    if (this.node === i) {
...
e.prototype._onTouchBegan = function (e, n) {
  if (0 != this.node.getComponent(B).scrollEnabled) {
    ...
    for (const o = 0; o < i.length; o++) {
      ...
      const a = r.getComponent(B);
      if (a) {
        const s = a.scrollView.getScrollOffset();
        s.x = -s.x;
...
t.prototype.layoutAttributesForElementsInRect = function (t) {
  for (const e = [], n = 0; n < this.attributes.length; n++) {
    const i = this.attributes[n];
...
e.prototype.update_reloadVisibleCellsIfNeeds = function () {
  if (
    (this.frameInterval <= 1 || this._frameIdx % this.frameInterval == 0) &&
    this._late_update_visible_data
  ) {
...
e.prototype.recycleInvisibleNodes = function (t) {
  ...
  const n = w;
  const i = this.scrollView.content.getContentSize();
  this.visibleNodesMap.forEach(function (o, r) {
    const a = o.getComponent(R);
...
e.prototype.reloadVisibleCells = function (t) {
  ...
  const e = this.layout.layoutAttributesForElementsInRect(t, this);
  const n = this.layout.shouldUpdateAttributesZIndex();
  if (n) {
...
e.prototype._reloadData = function () {
  ...
  if (this.reloadDataCounter <= 0) {
    ...
    const n = this.scrollView.getMaxScrollOffset();
    cc.Vec2.min(e, e, n);
...
e.prototype.onTouchItem = function (t) {
  if (this.onTouchItemAt) {
    const e = t.target.getComponent(R);
